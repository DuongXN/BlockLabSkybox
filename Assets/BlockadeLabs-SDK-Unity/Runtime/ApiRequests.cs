...
namespace BlockadeLabsSDK
{
    public class ApiRequests
    {
        ...
        public static async Task<string> CreateSkybox(List<SkyboxStyleField> skyboxStyleFields, int id, string apiKey, string drawingData)
        {
            // Create a dictionary of string keys and dictionary values to hold the JSON POST params
            // Tạo một dictionary để chứa các thông tin cần gửi đi
            Dictionary<string, string> parameters = new Dictionary<string, string>();

            // Thêm thông tin về ID Style vào dictionary
            parameters.Add("skybox_style_id", id.ToString());

            // Thêm thông tin về nét vẽ vào dictionary (chuyển linePoints thành chuỗi JSON)
            // string linePointsJson = JsonConvert.SerializeObject(drawingData);
            // parameters.Add("line_points", linePointsJson);
            parameters.Add("drawing_data", drawingData);
